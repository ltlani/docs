...
Slack is dedicated-only. Each Verb owner creates an app and provides its Client
ID, Client Secret, and Signing Secret. Verba encrypts those credentials and the
OAuth bot token. Never paste them into a chat or knowledge entry.
