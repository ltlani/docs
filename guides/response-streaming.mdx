...
Turn on **Response Streaming** in **Dashboard -> Verb -> AI Engine**. On
Discord, WhatsApp, and Telegram, Verba sends the first useful part of a reply
as soon as it is available, then edits that same message until the final text
is complete.
...
Enabling the platform toggle does not enable API SSE on a lower plan.
