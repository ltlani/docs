...
<AccordionGroup>
  <Accordion title="What is the Slack dedicated-app flow?">
    Create one Slack app for the Verb, paste its Client ID, Client Secret, and
    Signing Secret into Verba, copy and apply the generated app manifest, then
    authorize the app with Slack OAuth and choose active channels.
  </Accordion>
  <Accordion title="Which Slack commands are installed?">
    `/activate-channel`, `/remove-channel`, `/reset`, `/generate`, `/dashboard`,
    and `/ping`. Voice audio is a normal-reply sidecar, not a Slack command.
  </Accordion>
