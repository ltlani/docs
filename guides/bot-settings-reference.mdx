...
- Setup requires the app's `xoxb-` Bot User OAuth Token and Signing Secret.
  Verba validates the bot identity, encrypts both values, and generates the
  Events API, Interactivity, and Slash Command callback URLs.
- Mentions and DMs work after installation. With autonomous mode enabled, the
  Verb may also reply naturally in channels where its Slack app is present.
- Expression Controls cover emoji reactions, relevant GIF uploads, known
  ...
  messages.
- Slash command toggles cover `/reset`, `/generate`, `/dashboard`, and `/ping`.
- Content and workspace capability bundles depend on the scopes installed on
  the owner's Slack app. After adding scopes, reinstall the app and reconnect
  its current bot token.
